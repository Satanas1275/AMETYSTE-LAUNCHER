...
class Downloader extends events_1.EventEmitter {
     ...
     */
    async downloadFileMultiple(files, size, limit = 1, timeout = 30000) {
    ...
    async checkMirror(baseURL, mirrors) {
        for (const mirror of mirrors) {
            // Normalize to ensure exactly one slash between mirror and baseURL
            const normalizedMirror = mirror.replace(/\/+$/, '');
            const normalizedBase = baseURL.replace(/^\/+/, '');
            let testURL = `${normalizedMirror}/${normalizedBase}`;
            // Corrige la concaténation accidentelle "filesinstances" => "files/instances"
            testURL = testURL.replace(/\/?filesinstances\/?/gi, '/files/instances/');
            // Évite les doubles slashes dans le chemin (sans toucher "http://")
